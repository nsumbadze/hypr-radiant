...
void OverlayRenderer::renderCurrentMonitor(double alpha) {
    ...
    drawRect(box, CHyprColor{0.000F, 0.000F, 0.000F, static_cast<float>(0.78 * backdropAlpha)}, damage, 0, true);
...
void OverlayRenderer::renderFrame(const WorkspaceWallFrame& frame, double alpha, const CRegion& damage) {
    ...
    const auto cardFill       = CHyprColor{0.078F, 0.084F, 0.106F, static_cast<float>(0.96 * contentAlpha)};
    const auto activeFill     = CHyprColor{0.094F, 0.104F, 0.132F, static_cast<float>(0.98 * contentAlpha)};
    const auto selectedFill   = CHyprColor{0.110F, 0.118F, 0.142F, static_cast<float>(0.98 * contentAlpha)};
    const auto emptyFill      = CHyprColor{0.039F, 0.043F, 0.058F, static_cast<float>(0.72 * contentAlpha)};
    const auto windowFill     = CHyprColor{0.106F, 0.114F, 0.140F, static_cast<float>(0.96 * contentAlpha)};
    const auto windowSelectedFill = CHyprColor{0.122F, 0.130F, 0.160F, static_cast<float>(0.96 * contentAlpha)};
    const auto accent         = CHyprColor{0.420F, 0.880F, 0.820F, static_cast<float>(0.96 * contentAlpha)};
    const auto selectAccent   = CHyprColor{0.980F, 0.740F, 0.280F, static_cast<float>(0.96 * contentAlpha)};
    const auto shadow         = CHyprColor{0.000F, 0.000F, 0.000F, static_cast<float>(0.45 * contentAlpha)};
    const auto selectedGlow   = CHyprColor{0.980F, 0.740F, 0.280F, static_cast<float>(0.12 * contentAlpha)};
    const auto windowRing     = CHyprColor{0.980F, 0.740F, 0.280F, static_cast<float>(0.14 * contentAlpha)};
    const auto dockBg         = CHyprColor{0.027F, 0.030F, 0.042F, static_cast<float>(0.72 * contentAlpha)};
    ...
    renderLabel("Workspaces", titleX, titleY, std::max(1.0, frame.bounds.width * 0.45), 18, contentAlpha, damage);
    if (!searchActive && hasContent) {
        ...
        renderLabel("\xe2\x86\x91\xe2\x86\x93\xe2\x86\x90\xe2\x86\x92 navigate \xc2\xb7 Enter activate \xc2\xb7 1-9 jump \xc2\xb7 / search \xc2\xb7 Esc close",
            helpX, frame.bounds.height - 16.0 - 24.0 + 4.0, helpWidth, 12, contentAlpha * 0.68, damage);
    }
    ...
    if (hasDock && mode == LayoutMode::Stage) {
        ...
        drawRect(dockBox, dockBg, damage, 16);
    }
    ...
    for (const auto& workspace : frame.workspaces) {
        ...
        const auto round        = compact ? 10 : 16;
        ...
        if (workspaceSelected) {
            ...
            drawRect(glowBox, selectedGlow, damage, round + 6);
        }
        ...
        if (workspaceSelected) {
            ...
            border.grad1      = Config::CGradientValueData{selectAccent};
            border.a          = selectAccent.a;
            border.round      = round;
            border.borderSize = 2;
        ...
        if (workspace.active) {
            drawRect(CBox{workspaceBox.x + workspaceBox.w - 30.0, workspaceBox.y + 16.0, 14.0, 14.0},
                CHyprColor{0.420F, 0.880F, 0.820F, static_cast<float>(0.14 * contentAlpha)}, damage, 7);
            drawRect(CBox{workspaceBox.x + workspaceBox.w - 27.0, workspaceBox.y + 19.0, 8.0, 8.0}, accent, damage, 4);
        }

        renderLabel(workspace.name, workspace.rect.x + (compact ? 10.0 : 18.0), workspace.rect.y + (compact ? 8.0 : 14.0),
            std::max(1.0, workspace.rect.width - (compact ? 20.0 : 36.0)), compact ? 11 : 15, contentAlpha, damage);

        if (workspace.empty && !compact)
            renderLabel("Empty", workspace.rect.x + 18.0, workspace.rect.y + 44.0, std::max(1.0, workspace.rect.width - 36.0), 12, contentAlpha * 0.42, damage);
        ...
        for (const auto& window : workspace.windows) {
            ...
            const auto windowRound = compact ? 6 : 10;
            const auto footerHeight = compact ? 0.0 : std::min(38.0, std::max(30.0, windowBox.h * 0.22));

            if (windowSelected) {
                drawRect(CBox{windowBox.x - 4.0, windowBox.y - 4.0, windowBox.w + 8.0, windowBox.h + 8.0}, windowRing, damage, windowRound + 4);
            }

            drawRect(windowBox, windowSelected ? windowSelectedFill : windowFill, damage, windowRound);
            ...
            if (!compact && windowBox.h > 88.0) {
                ...
                renderLabel(window.label, window.rect.x + 14.0, window.rect.y + window.rect.height - footerHeight + 8.0,
                    std::max(1.0, window.rect.width - 28.0), 12, contentAlpha, damage);
            } else {
                renderLabel(window.label, window.rect.x + (compact ? 8.0 : 12.0), window.rect.y + (compact ? 5.0 : 8.0),
                    std::max(1.0, window.rect.width - (compact ? 16.0 : 20.0)), compact ? 10 : 12, contentAlpha, damage);
            }
        }
    }
...
void OverlayRenderer::renderSearchPanel(const WorkspaceWallFrame& frame, double alpha, const CRegion& damage) {
    ...
    const auto panelFill      = CHyprColor{0.058F, 0.063F, 0.082F, static_cast<float>(0.97 * alpha)};
    const auto panelBorder    = CHyprColor{0.140F, 0.148F, 0.176F, static_cast<float>(0.60 * alpha)};
    const auto inputFill      = CHyprColor{0.108F, 0.115F, 0.144F, static_cast<float>(0.98 * alpha)};
    const auto rowFill        = CHyprColor{0.092F, 0.099F, 0.125F, static_cast<float>(0.82 * alpha)};
    const auto selectedFill   = CHyprColor{0.178F, 0.160F, 0.115F, static_cast<float>(0.98 * alpha)};
    const auto selectedAccent = CHyprColor{0.980F, 0.740F, 0.280F, static_cast<float>(1.00 * alpha)};
    const auto badgeWindow    = CHyprColor{0.160F, 0.136F, 0.090F, static_cast<float>(0.92 * alpha)};
    const auto badgeSpace     = CHyprColor{0.105F, 0.158F, 0.154F, static_cast<float>(0.92 * alpha)};
    ...
    drawRect(CBox{panelBox.x + 8.0, panelBox.y + 10.0, panelBox.w, panelBox.h},
        CHyprColor{0.0F, 0.0F, 0.0F, static_cast<float>(0.42 * alpha)}, damage, 24);
    drawRect(panelBox, panelFill, damage, 20, true);

    CBorderPassElement::SBorderData border;
    border.box        = panelBox;
    border.grad1      = Config::CGradientValueData{panelBorder};
    border.a          = panelBorder.a;
    border.round      = 20;
    border.borderSize = 1;
    g_pHyprRenderer->m_renderPass.add(makeUnique<CBorderPassElement>(border));

    drawRect(inputBox, inputFill, damage, 12);
    renderLabel(std::format("{}_", m_searchQuery), inputBox.x + 16.0, inputBox.y + 13.0,
        std::max(1.0, inputBox.w - 80.0), 16, alpha, damage);
    renderLabel(std::format("{} result{}", targets.size(), targets.size() == 1 ? "" : "s"),
        inputBox.x + inputBox.w - 60.0, inputBox.y + 17.0, 48.0, 11, alpha * 0.50, damage);
    ...
    for (std::size_t index = visibleStart; index < visibleEnd; ++index) {
        ...
        if (target.type == OverviewTargetType::Workspace) {
            ...
            drawRect(badgeBox, badgeSpace, damage, 8);
            renderLabel("SPACE", row.x + 31.0, row.y + 23.0, 46.0, 9, alpha * 0.78, damage);
            renderLabel(std::format("Workspace {}", name), row.x + 104.0, row.y + 8.0, row.w - 128.0, 14, alpha, damage);
            renderLabel("Switch to this workspace", row.x + 104.0, row.y + 30.0, row.w - 128.0, 11, alpha * 0.50, damage);
        } else if (const auto* window = findWindowCard(target.windowId)) {
            drawRect(badgeBox, badgeWindow, damage, 8);
            renderLabel("WINDOW", row.x + 26.0, row.y + 23.0, 56.0, 9, alpha * 0.78, damage);
            renderLabel(window->label, row.x + 104.0, row.y + 8.0, row.w - 128.0, 14, alpha, damage);
            renderLabel(std::format("Workspace {}", window->workspaceId), row.x + 104.0, row.y + 30.0, row.w - 128.0, 11, alpha * 0.50, damage);
        }
    }

    if (targets.empty()) {
        renderLabel("No results", inputBox.x + 4.0, geometry.resultsY + 18.0, inputBox.w - 8.0, 16, alpha * 0.78, damage);
        renderLabel("Try another window title, workspace name, or number", inputBox.x + 4.0, geometry.resultsY + 48.0,
            inputBox.w - 8.0, 11, alpha * 0.48, damage);
    }
    ...
    renderLabel("\xe2\x86\x91\xe2\x86\x93 select \xc2\xb7 Enter activate \xc2\xb7 Esc clear",
        ...
        panelBox.w - 56.0, 11, alpha * 0.48, damage);
}
