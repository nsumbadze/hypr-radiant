...
class GestureController {
  public:
    ...
    // Named fields: the old positional install() had three consecutive bool()-returning callbacks
    // (overviewActive, shelfVisible, enabled) that were transposable at the call site.
    struct Callbacks {
        BoolFn   enabled;
        IntFn    fingers;
        DoubleFn distance;
        BoolFn   overviewActive;
        BoolFn   shelfVisible;
        BeginFn  begin;
        UpdateFn update;
        EndFn    end;
